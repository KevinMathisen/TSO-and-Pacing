...
if [[ $# -ne 2 ]]; then
    echo "Usage: $0 <run_num> <speed_mbps> # 250, 500, 1000, 2000"
    exit 2
fi
...
setup_server() {
  "$SCRIPT_PATH"/setup-server-experiment.sh
}

setup_client() {
  ssh -o BatchMode=yes "$CLIENT_SSH" "sudo $SCRIPT_PATH/setup-client-experiment.sh $MBPS"
...
SCRIPT_PATH="$HOME/master/TSO-and-Pacing/scripts/paper"
...
TS="$(date +%Y%m%d)"
RUN_NAME="${TS}___${MBPS}_run_${RUN_NUM}"
...
ssh -o BatchMode=yes "$EXTERNAL_SSH" "sudo /receiver/benchmark.sh -d 2 --stamp header -o $BENCH_OUT" \
