...
// absolute xfer location for LSO packets?
 ...
 * 
 * notify_reset_state_xfer: 
 *      used in NOTIFY_PROC in packets sent out
 * 
 * nfd_in_data_compl_refl_in: 
 *          Issue DMA telling seq num of DMA that is completed
 * 
 * nfd_in_jumbo_compl_refl_in: 
 *      Issue DMA telling seq num of jumbo DMA that is completed
 *      
 */
void
notify_setup_visible(void)
{
    __assign_relative_register(&notify_reset_state_xfer,
                               NFD_IN_NOTIFY_RESET_RD);                         // allocate vars to xfer registers given number, e.g. xfer 0
    ...
    __implicit_write(&notify_reset_state_xfer);                                 // implicit write to prevent dead-code removal
 ...
 * 
 * Get emem work queue address, and initialize ordering signals
 */
void
notify_setup_shared()
{
#ifdef NFD_IN_WQ_SHARED                                                         // get emem work queue address
    ...
    /* Kick off ordering */                                                     // Initialize ordering signals so managers can start (i.e. managers will have these signals raised)
    reorder_start(NFD_IN_NOTIFY_MANAGER0, &msg_order_sig);                      
    reorder_start(NFD_IN_NOTIFY_MANAGER0, &get_order_sig);                      // get_order_sig -> allowed to fetch (get) from issued ring
    reorder_start(NFD_IN_NOTIFY_MANAGER1, &msg_order_sig);                      
    reorder_start(NFD_IN_NOTIFY_MANAGER1, &get_order_sig);                      // msg_order_sig -> allowed to enqueue packet desc(s)
}
 ...
 * 
 * Initialize wait_msk, next_ctx, and LSO ring location
 */
void
notify_setup(int side)
{
    dst_q = wq_num_base;
    wait_msk = __signals(&msg_sig0, &msg_sig1, &msg_order_sig);                 // initialize the wait mask to wait for 1. messages to complete reading, 2. that we have msg token
                                                                                //  as this is what we need for first cycle

    next_ctx = reorder_get_next_ctx_off(ctx(), NFD_IN_NOTIFY_STRIDE);           // get next ctx using round robin within each side, e.g. after ctx 0 we have 2
...
do {                                                                         \
    NFD_IN_LSO_CNTR_INCR(nfd_in_lso_cntr_addr,                               \
                         NFD_IN_LSO_CNTR_T_NOTIFY_ALL_PKT_DESC); /* K: these NFD_IN_LSO_CNTR_INCR() are just for statistics, can maybe remove? */ \
    ...
    if (batch_in.pkt##_pkt##.eop) {                                          \
        ...
        NFD_IN_ADD_SEQN_PROC;                                                   /* K: Here we add seqnum to pkt_tmp to let reorder engine know order to send */ \
        ...
        for (;;) {                                                           \  /* TODO: check if this  points to all TSO packets in one chunk*/
            ...
            lso_msg_copy(&lso_pkt, lso_xnum);                                   /* K: Here we copy LSO desc from xfer to lso_pkt, could copy it into local memory instead */ \
                                                                             \
            NFD_IN_LSO_CNTR_INCR(nfd_in_lso_cntr_addr,                       \
                    NFD_IN_LSO_CNTR_T_NOTIFY_ALL_PKT_FM_LSO_RING);           \
                                                                             \
            /* Wait for the jumbo compl seq to catch up to the encoded seq */  /* K: Jumbo completed seq is basically how many lso desc written to lso issue ring I think */ \
            copy_absolute_xfer(&jumbo_compl_seq, jumbo_compl_xnum);          \
            seqn_chk = lso_pkt.jumbo_seq - jumbo_compl_seq;                  \
            while (seqn_chk > 0) {                                           /* K: Wait for either one available LSO desc in LSO ring queue, or whole batch of issued desc in issued desc queue */\
                ctx_swap();                                                  \
                                                                             \
                copy_absolute_xfer(&jumbo_compl_seq, jumbo_compl_xnum);      \
                seqn_chk = lso_pkt.jumbo_seq - jumbo_compl_seq;              \
            ...
            if (lso_pkt.desc.eop) {                                          \
                ...
                NFD_IN_ADD_SEQN_PROC;                                        /* K: Here we add seqnum to pkt_tmp to let reorder engine know order to send */ \
                batch_out.pkt##_pkt##.__raw[0] = pkt_desc_tmp.__raw[0];      \
                batch_out.pkt##_pkt##.__raw[1] = (lso_pkt.desc.__raw[1] |    /* ---pacing---: instead of writing to batch_out (xwrite) can write to queue in case of pacing? (i.e. from xread -> lm)*/ \
                                                  notify_reset_state_gpr);   \
                batch_out.pkt##_pkt##.__raw[2] = lso_pkt.desc.__raw[2];      \
                batch_out.pkt##_pkt##.__raw[3] = lso_pkt.desc.__raw[3];      \
                _SET_DST_Q(_pkt);                                            \
                                                                             /* ---pacing---:
                                                                             if we should pace, read lso_pkt.lso_seq_cnt
                                                                                if first (1 or 0? atle use 1):      just send, dont add to queue
                                                                                else, calculcate departure time:    base_time + pace_gap* lso_seq_cnt       (if elapsed, also skip)

                                                                                    then calculcate index, and add packet description to queue 
                                                                                        should be able to write directly to queue in local mem, as packet desc is stores in GPR
                                                                             */ \
                                                                             \
                __mem_workq_add_work(dst_q, wq_raddr, &batch_out.pkt##_pkt,  /* ---pacing---: only call this when we DONT add to queue. */ \
            ...
            } else {                                                         /* K: This block is basically just checking for bugs*/ \
                NFD_IN_LSO_CNTR_INCR(nfd_in_lso_cntr_addr,                   \
        ...
        wait_msk &= ~__signals(&wq_sig##_pkt);                               \
        __implicit_write(&wq_sig##_pkt);                                     \
    }                                                                        /* ---pace---: could poll queue here, as it might take a long time each lso chunk we process (as it includes sync mem reads)
                                                                                    could possibly also just poll queue if we have handled LSO chunk, not if only normal Issued Desc
                                                                            */ \
} while (0)
        ...
        unsigned int jumbo_compl_xnum, unsigned int lso_xnum)
{
    ...
    wait_for_all(&get_order_sig);                                               // K: Swap out thread until we get "get" token, i.e. our signal raised 

                                                                                // ---pace---: when we have get token, could also use this to access pace_queue
                                                                                //      could therefore check if we have packets to send from queue. If more than NFD_IN_MAX_BATCH_SZ, could send as batch to work queue as well.
                                                                                //      remember: when writing to work queue, check signals
    ...
    if (num_avail >= NFD_IN_MAX_BATCH_SZ)
    {
        ...
        reorder_done_opt(&next_ctx, &get_order_sig);                            // K: raise fetch signal for next thread in ring (we dont yield so no risk of this thread interupting)

        ctm_ring_get(NOTIFY_RING_ISL, input_ring, &batch_in.pkt0,               
                     (sizeof(struct nfd_in_issued_desc) * 4), &msg_sig0);       // K: retrieve batch of issued desc
        ctm_ring_get(NOTIFY_RING_ISL, input_ring, &batch_in.pkt4,
                     (sizeof(struct nfd_in_issued_desc) * 4), &msg_sig1);

        __asm {                                                                 // K: update the served amount with batch size, then yield with wait_msk
            ctx_arb[--], defer[2];                                              // K:   wait_msk is for: read/fetch to finish, that we have msg, 
            local_csr_wr[local_csr_active_ctx_wakeup_events, wait_msk];         // K:     and wq (work queue) writes from previous cycle are complete 
            alu[*served, *served, +, NFD_IN_MAX_BATCH_SZ];
        }

        wait_msk = __signals(&wq_sig0, &wq_sig1, &wq_sig2, &wq_sig3,
                             &wq_sig4, &wq_sig5, &wq_sig6, &wq_sig7,            // K: Set wait_msk for next cycle for this thread/ctx
        ...
        reorder_done_opt(&next_ctx, &msg_order_sig);                            // K: let next thread write message (raise msg signal)
        ...
        __qc_add_to_ptr_ind(PCIE_ISL, qc_queue, QC_RPTR, n_batch,
                            NFD_IN_NOTIFY_QC_RD, sig_done, &qc_sig);            // K: NB: here we signal that TX descs are completed, which the drivers may use to then free skbs?
                                                                                // ---pace---: might need to change logic of this when we pace.

    } else if (num_avail > 0) {
        /* There is a partial batch - process messages one at a time. */
        unsigned int partial_served = 0;

        wait_msk &= ~__signals(&msg_sig1);                                      // K: remove msh_sig1 from wait_msk

        /* ctm_ring_get() uses sig_done */
        ctm_ring_get(NOTIFY_RING_ISL, input_ring, &batch_in.pkt0,
                     sizeof(struct nfd_in_issued_desc), &msg_sig0);

        wait_sig_mask(wait_msk);                                                // K: wait for fetch to complete (+ signals from previous run)
        __implicit_read(&wq_sig0);
        __implicit_read(&wq_sig1);
        __implicit_read(&wq_sig2);
        __implicit_read(&wq_sig3);
        ...
        for (;;) {
            ...
            while (num_avail <= partial_served) {                               // K: only exit once we have 1 (or more) packets available to fetch
                ctx_wait(voluntary);
            ...
            /* ctm_ring_get() uses sig_done */
            ctm_ring_get(NOTIFY_RING_ISL, input_ring, &batch_in.pkt0,           // K: fetch one packet/issued desc
                         sizeof(struct nfd_in_issued_desc), &msg_sig0);
            ...
            wait_sig_mask(wait_msk);                                            // K: wait for fetch to complete and that we have msg
            __implicit_read(&wq_sig0);
            __implicit_read(&msg_sig0);
        ...
        *served += NFD_IN_MAX_BATCH_SZ;
        reorder_done_opt(&next_ctx, &get_order_sig);                            // K: allow next thread to fetch
         ...
         * We have finished _NOTIFY_PROC() where we need to
         * lock out other threads. */
        reorder_done_opt(&next_ctx, &msg_order_sig);                            // K: allow next thread to msg
        ...
        /* Participate in ctm_ring_get ordering */
        reorder_done_opt(&next_ctx, &get_order_sig);                            // K: pass on get token 

        /* Participate in msg ordering */
        wait_for_all(&msg_order_sig);                                           // K: wait for msg token
        reorder_done_opt(&next_ctx, &msg_order_sig);                            // K: pass on msg token
    }
}

/* Call _notify() based on side */
__intrinsic void
notify(int side)
{
    if (side == 0) {
 ...
 * 
 * wait until we get "get" token, then pass it on
 * then wait until we get "msg" token, then pass it on
 */
__intrinsic void
notify_manager_reorder()
{
    /* Participate in ordering */
    wait_for_all(&get_order_sig);                       // K: wait for get signal to be passed to manager (yield)
    reorder_done_opt(&next_ctx, &get_order_sig);        // K: allow next thread to get
    wait_for_all(&msg_order_sig);                       // K: wait for msg signal to be passed to manager (yield)
    reorder_done_opt(&next_ctx, &msg_order_sig);        // K: allow next thread to msg
}
...
main(void)
{
    /* Perform per ME initialisation
        K:  IN this case, bind transfer registers for inter-ME communication  */
    notify_setup_visible();

    if (ctx() == 0) {
        // K: compute emem work queue address
        // K: also start reorder protocol, initializing ordering signals
        notify_setup_shared();
    }
    ...
    // K: manager + 3 workers for Issue0:
#ifdef NFD_IN_HAS_ISSUE0
        notify_setup(0);

        if (ctx() == NFD_IN_NOTIFY_MANAGER0) {  // K: Manager

            __xread struct nfd_in_lso_desc lso_pkt0;                            // K: where we place LSO segment desc we read from nfd_in_issued_lso_ring
            ...
            for (;;) { 
                notify_manager_reorder();                                       // K: pass along get and msg
                notify_manager_reorder();
                distr_notify(0);                                                // K: let Issue ME know sequence numbers we have served
            }
        } else {
            for (;;) {
                notify(0);                                                      // K: 3 worker threads runs this:
            }                                                           // K: fetch issued desc(s) from issued ring, generate packet desc, enqueue to work queue
        }
#else
        for (;;) {
            ctx_swap(kill);
    ...
    // K: manager + 3 workers for Issue1:
#ifdef NFD_IN_HAS_ISSUE1
        notify_setup(1);

        if (ctx() == NFD_IN_NOTIFY_MANAGER1) { // K: Manager
            for (;;) {
                notify_manager_reorder();
                notify_manager_reorder();
                distr_notify(1);
            ...
            for (;;) {
                notify(1);                                                      // K: 3 worker threads runs this
            }
        }
#else
        for (;;) {
