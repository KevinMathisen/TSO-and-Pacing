...
__shared __gpr uint64_t pq_ctm_tail = 0;
__shared __gpr uint32_t pq_ctm_len = 0;
...
do {                                                                    \
    ...
    if (!signal_test(&wq_sig##_pkt)) halt();                            \
                                                                        \
    /* Retrive packet from CTM head, then increment */                  \
    ctm_ptr = &ctm_pacing_queue[pq_ctm_head];                           \
...
do {                                                                         \
    ...
    if (batch_in.pkt##_pkt##.eop) {                                          \
        ...
        /* ======= Write to local memory ============================== */   \
                                                                             \
        /* Stall if queue full */                                            \
        if (pq_ctm_len >= PQ_CTM_LENGTH-1) {                                 \
            DEBUG(0xAAAA); halt();                                           \
        }                                                                    \
                                                                             \
        /* write packet to CTM queue*/                                       \
        ctm_ptr = &ctm_pacing_queue[pq_ctm_tail];                            \
        addr_hi = ((unsigned long long)ctm_ptr >> 8) & 0xff000000;           \
        addr_lo = ((unsigned long long)ctm_ptr & 0xffffffff);                \
        __asm {                                                              \
            mem[write, batch_out.pkt##_pkt##, addr_hi, <<8, addr_lo,         \
                            __ct_const_val(2)], sig_done[*wq_sig##_pkt]      \
                                                                             ...
                                                                             \
        pq_ctm_tail++;                                                       \
        if (pq_ctm_tail >= PQ_CTM_LENGTH) pq_ctm_tail = 0;                   \
                                                                             \
        /* Cant update length yet, as write not complete! */                 \
                                                                             \
        ...
        for (;;) {                                                           \
            ...
            __implicit_read(&wq_sig##_pkt);                                  \
                                                                             \
            /* Previous packet completed writing to CTM, update length!*/    \
            if (lso_wait_msk & __signals(&wq_sig##_pkt)) pq_ctm_len++;       \
                                                                             \
        ...
        unsigned int jumbo_compl_xnum, unsigned int lso_xnum)
{
    ...
    unsigned int _ctx = ctx();

    /* Reorder before potentially issuing a ring get */
    wait_for_all(&get_order_sig);

    /* 4 worker threads, 2 from each side, dequeues from CTM */
    ...
    if (num_avail >= NFD_IN_MAX_BATCH_SZ)
    {
        ...
        /* If previous work cycle has completed writing to CTM, update length!*/
        if (wait_msk & __signals(&wq_sig1)) {
            pq_ctm_len += 8;
        } else if (wait_msk & __signals(&wq_sig0)) {
            pq_ctm_len++;
        }
        ...
        for (;;) {
            ...
            /* Previous packet has completed writing to CTM, update length! */
            pq_ctm_len++;
        }
