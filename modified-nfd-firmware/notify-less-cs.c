...
do {                                                                \
    ...
    batch_out.pkt##_out##.__raw[3] = lm_batch_in[_pkt].__raw[3];    \
                                                                    \
    __mem_workq_add_work(dst_q, wq_raddr, &batch_out.pkt##_out,     \
                            out_msg_sz, out_msg_sz, sig_done,       \
                            &wq_sig##_out);                         \
} while (0)

#define _SEND_PACKET_LSO(_pkt, _out)                                \
do {                                                                \
    wait_for_all(&wq_sig##_out);                                    \
                                                                    \
    batch_out.pkt##_out##.__raw[0] = pkt_desc_tmp.__raw[0];         \
...
do {                                                                         \
    ...
    if (lm_batch_in[_pkt].eop) {                                             \
        ...
        switch (next_batch_out) {                                            \
            case 0: _SEND_PACKET(_pkt, 0); break;                            \
            case 1: _SEND_PACKET(_pkt, 1); break;                            \
            case 2: _SEND_PACKET(_pkt, 2); break;                            \
            case 3: _SEND_PACKET(_pkt, 3); break;                            \
            case 4: _SEND_PACKET(_pkt, 4); break;                            \
            case 5: _SEND_PACKET(_pkt, 5); break;                            \
            case 6: _SEND_PACKET(_pkt, 6); break;                            \
            case 7: _SEND_PACKET(_pkt, 7); break;                            \
        }                                                                    \
                                                                             \
        next_batch_out++;                                                    \
        next_batch_out &= 7;                                                 \
        ...
        for (;;) {                                                           \
            ...
            if (lso_pkt.desc.eop) {                                          \
                ...
                NFD_IN_ADD_SEQN_PROC;                                        \
                                                                             \
                switch (next_batch_out) {                                    \
                    case 0: _SEND_PACKET_LSO(_pkt, 0); break;                \
                    case 1: _SEND_PACKET_LSO(_pkt, 1); break;                \
                    case 2: _SEND_PACKET_LSO(_pkt, 2); break;                \
                    case 3: _SEND_PACKET_LSO(_pkt, 3); break;                \
                    case 4: _SEND_PACKET_LSO(_pkt, 4); break;                \
                    case 5: _SEND_PACKET_LSO(_pkt, 5); break;                \
                    case 6: _SEND_PACKET_LSO(_pkt, 6); break;                \
                    case 7: _SEND_PACKET_LSO(_pkt, 7); break;                \
                }                                                            \
                                                                             \
                next_batch_out++;                                            \
                next_batch_out &= 7;                                         \
